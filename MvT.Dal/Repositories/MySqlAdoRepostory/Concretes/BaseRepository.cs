    ...
    public class BaseRepository<T> : IRepository<T> where T : class
    {
        ...
        public Task<List<T>> GetAll()
        {
            ...
            try
            {
                ...
                var returnRec = _MySqlDatabaseManager.GetAll(_dbTransaction, _selectColums, _joinTable);
                if (transactionCheck) CommitTransaction();
                return returnRec;
            }
            catch (Exception ex)
        ...
        public Task<List<T>> GetModifieds()
        {
            ...
            try
            {
                if (_dbTransaction == null)
                    throw new ArgumentException("MySqlAdoBaseRepository\nTransaction cannot be null.");
                var returnRec = _MySqlDatabaseManager.GetModifieds(_dbTransaction, _selectColums, _joinTable);
                if (transactionCheck) CommitTransaction();
                return returnRec;
            }
            catch (Exception ex)
